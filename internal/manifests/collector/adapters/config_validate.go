...
func getEnabledComponents(config map[string]interface{}, componentType ComponentType) map[interface{}]bool {
	...
	for compID := range components {
		...
		availableComponents[compID] = false
	}

	cfgService, withService := config["service"]
	if !withService {
		return nil
	}

	pipeline, withPipeline := cfgService["pipelines"].(map[string]interface{})
	if !withPipeline {
		return nil
	}
